...
tab1, tab2, tab3, tab4, tab5 = st.tabs(["📊 Dashboard", "📦 Stock Views", "📈 Pareto & Stock-Out", "🧊 Stale Inventory", "🛒 My Shopping List"])
...
with tab2:
    ...
    # Import PinnedShoppingListManager
    from data_loader import PinnedShoppingListManager
    pinned_manager = PinnedShoppingListManager()
    pinned_items = pinned_manager.list_pinned_items()
    pinned_products = {item["product"] for item in pinned_items}
    ...
    shop_pinned = shop[shop["Product"].isin(pinned_products)].copy()
    shop_unpinned = shop[~shop["Product"].isin(pinned_products)].copy()

    # Sort both groups
    shop_pinned = shop_pinned.sort_values(["Qty Sold","Item Price"], ascending=[False, True])
    shop_unpinned = shop_unpinned.sort_values(["Qty Sold","Item Price"], ascending=[False, True])

    # Display pinned items first if any
    if not shop_pinned.empty:
        st.markdown("#### Pinned Items")

        # Create AgGrid for pinned items with selection
        gb_pinned = GridOptionsBuilder.from_dataframe(shop_pinned)
        gb_pinned.configure_selection(selection_mode="multiple", use_checkbox=True)
        gb_pinned.configure_column("Qty Sold", valueFormatter="value != null ? Math.round(value) : ''")
        gb_pinned.configure_column("Inventory Left", valueFormatter="value != null ? Math.round(value) : ''")
        gb_pinned.configure_column("Item Price", valueFormatter="value != null ? '$' + value.toFixed(2) : ''")
        gb_pinned.configure_column("Sold %", valueFormatter="value != null ? value.toFixed(1) + '%' : ''")
        gb_pinned.configure_grid_options(domLayout="normal")
        pinned_opts = gb_pinned.build()

        grid_pinned = AgGrid(
            shop_pinned,
            gridOptions=pinned_opts,
            update_mode=GridUpdateMode.SELECTION_CHANGED,
            height=min(200, 50 + len(shop_pinned) * 35),
            fit_columns_on_grid_load=True,
            theme="alpine",
        )

        if st.button("Unpin selected items", key="unpin_btn"):
            selected_pinned = grid_pinned["selected_rows"]
            if selected_pinned:
                for row in selected_pinned:
                    product = row.get("Product")
                    if product:
                        pinned_manager.unpin_item(product)
                st.success(f"Unpinned {len(selected_pinned)} item(s)")
                st.rerun()
            else:
                st.warning("Select items to unpin")

        st.divider()

    # Display unpinned items
    st.markdown("#### All Items" if shop_pinned.empty else "#### Other Items")

    # Create AgGrid for unpinned items with selection
    gb_shop = GridOptionsBuilder.from_dataframe(shop_unpinned)
    gb_shop.configure_selection(selection_mode="multiple", use_checkbox=True)
    gb_shop.configure_column("Qty Sold", valueFormatter="value != null ? Math.round(value) : ''")
    gb_shop.configure_column("Inventory Left", valueFormatter="value != null ? Math.round(value) : ''")
    gb_shop.configure_column("Item Price", valueFormatter="value != null ? '$' + value.toFixed(2) : ''")
    gb_shop.configure_column("Sold %", valueFormatter="value != null ? value.toFixed(1) + '%' : ''")
    gb_shop.configure_grid_options(domLayout="normal")
    shop_opts = gb_shop.build()

    grid_shop = AgGrid(
        shop_unpinned,
        gridOptions=shop_opts,
        update_mode=GridUpdateMode.SELECTION_CHANGED,
        height=400,
        fit_columns_on_grid_load=True,
        theme="alpine",
    ...
    if st.button("Pin selected items", key="pin_btn"):
        selected_shop = grid_shop["selected_rows"]
        if selected_shop:
            for row in selected_shop:
                product = row.get("Product")
                barcode = row.get("Barcode", "")
                sku = row.get("SKU", "")
                if product:
                    pinned_manager.pin_item(product, barcode, sku)
            st.success(f"Pinned {len(selected_shop)} item(s)")
            st.rerun()
        else:
            st.warning("Select items to pin")
...
# =========================
# TAB 5: MY SHOPPING LIST (Mobile-Optimized)
# =========================
with tab5:
    from data_loader import PinnedShoppingListManager
    ...
    if not shopping_list:
        ...
        checked_items = [item for item in shopping_list if item.get("checked")]
        ...
        if checked_items:
            st.markdown("---")
            st.caption(f"✅ Checked Items ({len(checked_items)})")
